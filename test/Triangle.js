class Triangle {
  ...
  render() {
    ...
    if (g_funMode) {
      this.colorPhase += g_spinRate/20;
      const t = this.colorPhase;

      // https://krazydad.com/tutorials/makecolors.php
      rgba = [
        Math.sin(t) * 0.5 + 0.5,
        Math.sin(t + (2 * Math.PI) / 3) * 0.5 + 0.5,
        Math.sin(t + (4 * Math.PI) / 3) * 0.5 + 0.5,
        1.0
      ];
    }
    // Draw
...
function drawColoredTriangle(vertices, hexColor) {
  if (vertices.length !== 6) {
    console.error("drawColoredTriangle expects 6 values (3 vertices)");
    return;
  }

  let rgba = hexToRgba(hexColor);
